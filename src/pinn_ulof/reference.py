"""Stiff reference solver for the axial channel.

Method of lines: :mod:`pinn_ulof.physics` discretises Chapter 3's
energy equations in ``z``, and an implicit Radau integrator advances the
resulting ODE system in ``t``. This is the **held-out truth**: the network never sees
it during training, only at test time.
...
Removing the prompt neutron mode (deviation D-KIN-1) buys a factor of ~2300, which
still leaves a stiff problem — hence Radau.
...
def solve_reference(
    ...
    *,
    ...
    constant 1.0, which is the open-loop configuration used throughout.

    **The run terminates when any temperature reaches ``T_limit``**, which
    defaults to :data:`~pinn_ulof.sodium.T_MAX`, the upper bound
...
def energy_balance(
    ...
    **The amplitude comes from the trajectory, not from a default of one.**
    ...
    # of exactly the vapour outflow -- about 45 W against 50 kW here, i.e. 9e-4, which
    # is the level the closure floors at without it.
