...
manual are registered in ``docs/02-model.md``.

Units are **SI** throughout (m, kg, s, K, W, Pa). Reactivity is dimensionless
absolute (same units as ``beta_eff``), never dollars: mixing the two is a
factor-``beta`` error, i.e. about 286x here.
...
residuals live in ``physics``, the solver in ``reference``, and the network in
``network``.
"""
...
#: Six-group delayed-neutron decay constants [1/s] and U-235 group fractions. These are
#: nuclear data rather than model choices, which is why they live beside the parameters
#: that use them rather than in a configuration file.
...
class AxialParams:
    ...
    radially lumped into one node per axial position; the deviation register in
    ``docs/02-model.md`` gives the justification.
    """
    ...
    # U-235 six-group data, rescaled to the SFR beta_eff.
    ...
    # identically and the reactor is exactly critical -- a consequence of the
    # logarithmic Doppler, which needs no fitted offset to achieve it.
    rho_ext: float = 0.0
    ...
    # the whole coupled system, which the optimiser can and does find. With it
    # non-zero, `psi_h` does not vanish when `psi_f` does. Typical
    ...
    # Condensation coefficient (manual section 12.5). Held at zero, which is what makes
    # the algebraic void closure valid: with no condensation the vapour source is
    # non-negative, so the void is monotone along a characteristic and is a function of
    # the coolant temperature alone.
    condensation: float = 0.0

    # --- Flow (manual Eq. 5.3-61 analogue; see deviation D-FLOW-1) ---------
    # Let vapour generation accelerate the mixture. Held off: Eq. 3.9-1 gives the flow as
    # independent of height, which is the form used throughout this work.
    ...
    def __post_init__(self) -> None:
        ...
        self._validate()
        # Rescale the U-235 relative six-group spectrum to this beta_eff.
    ...
    def alpha_D(self, void_fraction: Scalar) -> Scalar:
        ...
        """
        # Backend-agnostic: called from inside the residual, where `void_fraction`
        # is a JAX array that must stay differentiable.
