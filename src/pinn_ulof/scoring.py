...
``predict`` returns plain numpy arrays, so nothing here needs JAX. Every published
number comes through these functions; a metric whose definition lives in more than
one place drifts, and the drift is invisible.

No training module imports this, so nothing in a loss can reach the reference.
"""
...
if TYPE_CHECKING:
    ...
    from pinn_ulof.reference import AxialTrajectory
