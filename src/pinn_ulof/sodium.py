...
Eq. 12.13-1 to 12.13-8 of the SAS4A/SASSYS-1 manual (ANL/NSE-SAS/5.8.1,
https://sas-doc.nse.anl.gov/latest/Part04/Ch12/12.13.html), transcribed with their
equation numbers: the eight relations this channel model evaluates. Eq. 12.13-9 to
12.13-13 -- compressibility, expansion, conductivity, viscosity and saturated liquid
enthalpy -- are not implemented, because nothing here calls them. At 1 atm the saturation
temperature is ~1154 K.

**One implementation, two backends.** Every function dispatches on its argument
type, so numpy arrays and JAX arrays evaluate the *same* expression tree rather
than two transcriptions that could drift apart. The pure-polynomial correlations
are **bit-identical** across both, since IEEE-754 pins ``+`` and ``*``; the ones
using ``exp``, ``log`` or division agree to **~1 ULP**, because the two call
different libm implementations and IEEE-754 does not require correctly-rounded
transcendentals. All functions are pure, closed-form and differentiable over the
validity range, so they can sit inside a residual and be differentiated by
``jax.grad``.

Float64 is required: ``pinn_ulof/__init__.py`` sets
``jax.config.update("jax_enable_x64", True)`` at import. At float32 these
correlations lose about eight significant digits and the ``Ts(Ps(T))`` round trip
degrades from 1e-11 K to ~1e-2 K.
...
avoid fitting the rapid near-critical variation. :func:`liquid_heat_capacity` is
built from powers of ``1/(T_c - T)`` and diverges at the critical point. **Nothing here
clamps or raises** — a hard guard would break autodiff and abort training on a
transient excursion. Call :func:`in_range` if you need to check.
